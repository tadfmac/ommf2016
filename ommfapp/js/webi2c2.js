  ...
  **/
  read8: function (readRegistar) {
    return new Promise((resolve, reject) => {
      ...
      window.WorkerOvserve.observe(`i2c.read.${this.portNumber}.${readRegistar}`, (data) => {
        resolve(data.value);
        window.WorkerOvserve.delete(`i2c.read.${this.portNumber}.${readRegistar}`); // @@@@@
      });
  ...
  write8: function (registerNumber, value) {
    return new Promise((resolve, reject) => {
      ...
      window.WorkerOvserve.observe(`i2c.write.${this.portNumber}.${registerNumber}`, (data) => {
        resolve(data.value);
        window.WorkerOvserve.delete(`i2c.write.${this.portNumber}.${registerNumber}`); // @@@@@
      });
    });
  },
